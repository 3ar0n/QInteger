...
QInt operator * (QInt a, QInt b)
{
	...
	*/
	...
	else
	{
		...
		bool P, Q = 0x0;
		for (int j = 0; j < ArraySize * Byte; j++)
		{
			P = getBit(a.data[ArraySize - 1], 0);
			if (P && !Q)		// PQ = 10
				D = D - b;
			else if (!P && Q)	// PQ = 01
				D = D + b;
			...
			*/
			...
			a.data[0] = a.data[0] >> 1;
			a.data[0] = a.data[0] + (D.data[ArraySize - 1] & 0x1) * 128;
	...
	*/
	...
	else
	{
		...
		for (int j = 0; j < ArraySize * Byte; j++)
		{
			...
			c.data[ArraySize - 1] = c.data[ArraySize - 1] << 1;
			c.data[ArraySize - 1] += (a.data[0] >> 7) & 0x1;
			for (int i = 0; i < ArraySize - 1; i++)
			{
				a.data[i] = a.data[i] << 1;
				a.data[i] += (a.data[i + 1] >> 7) & 0x1;
			}
			a.data[ArraySize - 1] = a.data[ArraySize - 1] << 1;
...
QInt operator << (QInt a, int shift)
{
	...
	*/
	if (!a.overflow)
	{
		...
		{
			for (int j = ArraySize - 1; j >= 0; j--)		// dịch trái từng phần tử (8 bits) của QInt từ sau lên trước
			{
	...
	*/
	if (!a.overflow)
	{
		...
		for (int i = 0; i < shift; i++)				// mỗi lần dịch phải 1 bit
		{
			...
			for (int j = 0; j < ArraySize; j++)			// dịch phải từng phần tử (8 bits) của QInt từ trước ra sau
			{
				if (a.data[j] % 2 != 0)
					_borrow = 128;
				else
